...
from NodeGraphQt.base.commands import (NodeAddedCmd, NodeMovedCmd,
                                       NodeRemovedCmd, PortConnectedCmd)
...
from NodeGraphQt.constants import (IN_PORT, NODE_LAYOUT_DIRECTION,
                                   ...
                                   PIPE_LAYOUT_STRAIGHT, URI_SCHEME,
                                   URN_SCHEME, VIEWER_GRID_LINES)
...
class NodeGraph(QtCore.QObject):
    ...
    def set_pipe_style(self, style=PIPE_LAYOUT_CURVED):
        ...
        """
        pipe_max = max([PIPE_LAYOUT_CURVED, PIPE_LAYOUT_STRAIGHT, PIPE_LAYOUT_ANGLE])
    ...
    def create_node(
        self, node_type, name=None, selected=True, color=None, text_color=None, pos=None
    ):
        """
    ...
    def _serialize(self, nodes):
        ...
        for n_id, n_data in nodes_data.items():
            ...
            for pname, conn_data in outputs.items():
                for conn_id, prt_names in conn_data.items():
                    for conn_prt in prt_names:
                        pipe = {OUT_PORT: [n_id, pname], IN_PORT: [conn_id, conn_prt]}
                        if pipe not in serial_data["connections"]:
                            serial_data["connections"].append(pipe)
            ...
            if NodeCls:
                ...
                if isinstance(node, SubGraph):
                    ...
                    if not published:
                        ...
                        if sub_graph:
                            children = self._deserialize(
                                sub_graph, relative_pos, pos, False
                            )
    ...
    def save_session(self, file_path):
        ...
        serialized_data["graph"] = {
            "node_space": node_space,
            "pipe_layout": self._viewer.get_pipe_layout(),
        }
