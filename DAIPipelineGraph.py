...
class DAIPipelineGraph:
    def __init__(self, path):
        ...
        for node_id in data["nodes"]:
            ...
            if node_type in self.createNode:
                ...
                print(
                    "DAIPipelineGraph Error: Unknown Node Type: "
                    + node_type
                    + ", skipping it."
                )
    ...
    def CreateColorCamera(self, node_data):
        ...
        node.setImageOrientation(
            getattr(dai.CameraImageOrientation, node_data["orientation"])
        )
        node.setResolution(
            getattr(dai.ColorCameraProperties.SensorResolution, node_data["resolution"])
        )
    ...
    def CreateMonoCamera(self, node_data):
        ...
        node.setResolution(
            getattr(dai.MonoCameraProperties.SensorResolution, node_data["resolution"])
        )
        return node
    ...
    def CreateStereoDepth(self, node_data):
        ...
        node.initialConfig.setMedianFilter(
            getattr(dai.MedianFilter, node_data["median_filter"])
        )
        node.initialConfig.setConfidenceThreshold(
            int(node_data["confidence_threshold"])
        )
        return node
    ...
    def CreateFeatureTracker(self, node_data):
        ...
        node.setHardwareResources(
            int(node_data["num_shaves"]), int(node_data["num_memory_slices"])
        )
        node.setWaitForConfigInput(node_data["wait_for_config_input"])
        return node
    ...
    def CreateObjectTracker(self, node_data):
        ...
        node.setTrackerIdAssigmentPolicy(
            getattr(dai.TrackerIdAssigmentPolicy, node_data["id_policy"])
        )
